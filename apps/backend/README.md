...
worldcup2022 is a customized DApp written in Python, which originally resembles the one provided by the sample [Echo Python DApp](https://github.com/cartesi/rollups-examples/tree/main/echo-python).
...
```shell
DAPP_NAME=worldcup2022 docker compose -f ./deploy-testnet.yml up
```

This will create a file at `./deployments/<network>/worldcup2022.address` with the deployed contract's address.
Once the command finishes, it is advisable to stop the docker compose and remove the volumes created when executing it.

```shell
DAPP_NAME=worldcup2022 docker compose -f ./deploy-testnet.yml down -v
```
...
```shell
yarn start input send --payload "Hello there" --addressFile path/to/worldcup2022/deployments/<network>/worldcup2022.address
```
...
ROLLUP_HTTP_SERVER_URL="http://127.0.0.1:5004" python3 worldcup2022.py
```
...
```shell
ls *.py | ROLLUP_HTTP_SERVER_URL="http://127.0.0.1:5004" entr -r python3 worldcup2022.py
```
